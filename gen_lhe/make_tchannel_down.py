...
prefix = "tch_associated_down" # Prefix that the MG job was generated with
...
for i in [500,1000,5000,8000]:
	...
	batchn += "sed -i 's/500000	= nevents/"+str(nevt_each)+" = nevents/g' ../Cards/run_card.dat\n" # Change events in MG run card
	...
	batchn += "sed -i 's/1000022/4900101/g;n' ../Events/run_01/unweighted_events.lhe\n" # Change events in MG run card	
	batchn += "sed -i 's/1000022/-4900101/g' ../Events/run_01/unweighted_events.lhe\n" # Change events in MG run card	
	batchn += "cd /group/hepheno/smsharma/Dark-Showers/gen/\n" # Go to Pythia script folder
	# batchn += "python SubmitSigRunsTchannelDown.py\n" # Run python file to submit jobs for various rinv
	# batchn += "./monojet.exe -m lhe -w -i " + mg_dir + prefix + "_" + str(i) + "/Events/run_01/unweighted_events.lhe -metmin 0 -n 500000 -o tchannel_mphi_" + str(i) + "-phimass " + str(i) + "-lambda " + str(i/2) + "\n"
