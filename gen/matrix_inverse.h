...
template <class D> class MatrixClass{
  ...
  MatrixClass() {};                  // private ctor's
  MatrixClass(int newmaxsize) {MatrixClass(newmaxsize,newmaxsize);};
public:
  MatrixClass(int newmaxsize, int newactualsize)  { // the only public ctor
  ...
  void settoproduct(MatrixClass& left, MatrixClass& right)  {
  ...
  void copymatrix(MatrixClass&  source)  {
  ...
  float invert()  {
    ...
    /*
    cout<<"Outputting matrix to be inverted: "<<endl;
    ...
    */  
    ...
    if (actualsize <= 0) return 0;  // sanity check
    if (actualsize == 1) 
      {
	float determinant=data[0];
	data[0]=1.0/data[0];
	return determinant; 
      } 
    ...
    //intercept to get determinant
    float det=1;
    
    for(int i=0; i<maxsize; i++)
      det*=data[i*maxsize+i];
    ...
    /*
    cout<<"Outputting inverted matrix: "<<endl;
    for(int i=0; i<maxsize; i++)
      {
	for(int j=0; j<maxsize; j++)
	  {
