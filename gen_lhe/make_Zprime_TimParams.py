...
prefix = "DM_Zprime_TimParams" # Prefix that the MG job was generated with
...
for MZ in masses:
	...
	batchn += "sed -i 's/{MASS} # MY1/"+str(MZ)+" # MY1/g' ../Cards/param_card.dat\n" # Change seed in MG run card
	W = MZ/3*(2*2*gX**2 + 6*3*gq**2)/(4*np.pi) # Calculate width
	batchn += "sed -i 's/{WIDTH} # WY1/"+str(W)+" # WY1/g' ../Cards/param_card.dat\n" # Change seed in MG run card
	...
	batchn += "sed -i 's/5000001/4900101/g' ../Events/run_01/unweighted_events.lhe\n" # Change events in MG run card	
