...
for M in np.arange(300,3100,100):
    for rinv in rinv_ary:    
        fname_tag = "/group/hepheno/hlou/pair_prod_"+str(M)+".lhe -metmin 0 -n 200000 -phimass 10 -lambda 5 -v "
        rinv_tag = "-inv " + str(rinv)
        out_tag = "-o " + "tchannelEventsPairProd/pair_prod_"+str(M)+"_rinv" + str(rinv)
        tag = fname_tag + " " + rinv_tag + " " + out_tag
        print tag
        batchn = batch + tag
        fname = "batch/run_tch_"+str(M)+"_"+str(rinv)+".batch"
        f=open(fname, "w")
        f.write(batchn)
        f.close()
